...
// AtomicMatchBundle contains a match and a transaction that the client can submit on-chain
type AtomicMatchBundle struct {
	MatchResult  ApiExternalMatchResult `json:"match_result"`
	SettlementTx SettlementTransaction  `json:"settlement_tx"`
}

// ApiExternalMatchResult is the result of a request to generate an atomic match
...
// SettlementTransaction is an EVM transaction parameterization for settling an atomic match
