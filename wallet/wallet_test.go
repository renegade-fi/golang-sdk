...
func TestWalletReblind(t *testing.T) {
	...
	assert.Equal(
		t, balance, walletShare.Balances[0],
		"Balance not correctly represented after reblinding",
	)

	// Check if the order is correctly represented
	assert.Equal(
		t, order.BaseMint, walletShare.Orders[0].BaseMint,
		"Order BaseMint not correctly represented after reblinding",
	)
	assert.Equal(
		t, order.QuoteMint, walletShare.Orders[0].QuoteMint,
		"Order QuoteMint not correctly represented after reblinding",
	)
	assert.Equal(
		t, order.Amount, walletShare.Orders[0].Amount,
		"Order Amount not correctly represented after reblinding",
	)
	assert.Equal(
		t, order.Side, walletShare.Orders[0].Side,
		"Order Side not correctly represented after reblinding",
	)
	assert.Equal(
		t, order.WorstCasePrice, walletShare.Orders[0].WorstCasePrice,
